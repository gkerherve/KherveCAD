...
class Photo3DDialog(QDialog):
    def __init__(self, window):
        ...
        self.window_ = window
        self.setWindowTitle("Mesh from Photo")
        self.setWindowFlag(Qt.Tool, True)
        self.worker = None
        form = QFormLayout()
        row = QHBoxLayout()
        self.image = QLineEdit()
        self.image.setPlaceholderText("A picture of the object, one view")
        browse = QPushButton("Browse\u2026")
        browse.clicked.connect(self._browse)
        row.addWidget(self.image, 1)
        row.addWidget(browse)
        form.addRow("Picture", row)
        ...
        form.addRow("Generator", self.backend)
        self.key = QLineEdit()
        ...
        form.addRow("Longest side", self.size)
        self.log = QPlainTextEdit()
        self.log.setReadOnly(True)
        self.log.setMaximumBlockCount(200)
        self.go = QPushButton("Generate and import")
        self.go.clicked.connect(self.generate)
        lay = QVBoxLayout(self)
        lay.addLayout(form)
        lay.addWidget(QLabel("One picture gives a plausible surface — the "
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Picture", "", "Images (*.png *.jpg *.jpeg *.webp)")
        if path:
    ...
    def generate(self):
        ...
        if not Path(image).is_file():
            self.say("Choose a picture first.")
            return
        ...
        self.say(f"Generating with {BACKEND_LABELS[backend]}\u2026")
    ...
    def _done(self, stl):
        self.go.setEnabled(True)
        self.say(f"Imported {Path(stl).name}")
