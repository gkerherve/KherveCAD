...
~/Documents/KherveCAD Library, one folder per AREA of work (Sport, House \
& home, Trees & plants, Molecules, Unit cells & crystals...); list_parts \
searches them too, descriptions included, under 'My library: <area>' — \
reuse one before modelling again. Every Object created or changed in \
the window is SAVED AUTOMATICALLY a few seconds later, titled by its \
name — so name every Object descriptively (what it is + key size, never \
'Object 2', which is skipped). When a design looks right, call \
save_to_library on it with its area, a descriptive title and a real \
DESCRIPTION (purpose, dimensions, parameters and what they change, \
construction, how to reuse) plus tags: later autosaves keep that text, \
and it is how the next request finds the part.
