...
class DocumentModel(QObject):
    ...
    def global_assigns(self):
        ...
        "Global" scope of the Variables sheet."""
        out = []
    ...
    def to_scad_map(self, only: CadNode = None):
        """The full program plus ``{node_id: (first, last_exclusive)}``
        line spans into it — the basis for code-line highlighting and
        for mapping OpenSCAD error line numbers back to nodes. With
        *only*, the program is the document's top-level variables plus
        that subtree alone (the isolated-Object view)."""
